            ...
            <b-col cols="auto">
                ...
                <modal
                    ...
                    <div class="form-emails-share punku-box pt-5 pb-4">
                        ...
                        <p>Comparte tu anuncio con tus contactos</p>
    ...
    methods: {
        ...
        sendEmail () {
            ...
            if (!this.$v.emails.$invalid) {
                ...
                GigsService.shareEmails(this.emails.items)
                .then((resp) => {
                    this.emails.isLoading = false
                    this.$modal.hide('share-email')
                    this.emails.items = []
                    setTimeout(() => {}, 500)
                    this.$bus.emit("notifications:push", { title: 'Excelente!', type: 'dark', message: 'El anuncio fue compartido exitosamente'})
                })
                .catch ((err) => {
                    this.emails.isLoading = false
                    this.$bus.emit("notifications:push", { type: 'error', message: 'Ocurrio un error al tratar de enviar los email'})
                })
            } else {
